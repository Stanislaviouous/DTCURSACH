// Если хеш делиться на 103
// Хещ определяеться функцией "textToBinarySum"

// Переводит данные в двоичный код и складывает
const textToBinarySum = (str = '') => {
    let res = '';
    res = str.split('').map(char => {
       return  parseInt(char.charCodeAt(0).toString(2), 10);
    });
    return (res.reduce((sum, current) => sum + current, 0) + "");
};
// $env:HTTP_PORT=3001; $env:P2P_PORT=6001; npm start
// $env:HTTP_PORT=3002; $env:P2P_PORT=6002; $env:PEERS="ws://?:6001"; npm start
...
var calculateHash = (index, previousHash, timestamp, data, nonce) => {
    return textToBinarySum(index + previousHash + timestamp + data + nonce)
...
var mineBlock = (blockData) => {
    ...
    // Изменить
    while (
        parseInt(nextHash, 10) % 103 !== difficulty
        ){
            nonce++;
